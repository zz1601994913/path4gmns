...
def _output_accessibility(min_travel_times, zones, mode='p', output_dir='.'):
    ...
    with open(output_dir + '/accessibility.csv', 'w', newline='') as f:
        ...
        for k, v in min_travel_times.items():
            ...
            coord_oz = zones[k[0]]  # zones={}key:ID,value:质点坐标
            coord_dz = zones[k[1]]
...
def evaluate_accessibility(ui,
    ...
    multimodal
        ...
        对或错。其默认值为True。它只会影响可访问性_aggregated.csv的输出。
如果为True，则将对settings.yml中定义的所有agent进行可访问性评估。给定预算时间（最多240分钟），每个定义模式下每个区域的可访问区域数将输出到accessibility_aggregated.csv。
如果为False，则访问性评估将仅针对目标agent进行。给定预算时间（最多240分钟），目标模式下每个区域的可访问区域数将输出到accessibility_aggregated.csv。
    ...
    time_dependent
        ...
        真：可达性将使用所选时期link的自由流旅行时间，换言之，可达性只与所选时期有关，与agnettype无关
        假：可达性将使用路径长度/每个agnet的自由流速度
    ...
    if multimodal:
        ...
        for at in ats:  # 遍历每一个agent更新min_tt，并找到所有agent中max_min最大的
        ...
        max_min = _update_min_travel_time(an,
                                          ...
                                          demand_period_id)
        ats = [at]
    # 以MIN_TIME_BUDGET+BUDGET_TIME_INTVL为间隔划分区域，起始区域为[MIN_TIME_BUDGET,MIN_TIME_BUDGET+BUDGET_TIME_INTVL]→1,
    # 所需的interval的数量是最远距离所处区域+1
...
def _update_min_travel_time(an, at, min_travel_times, time_dependent, demand_period_id):  # 返回离得最远的两个区域质心间的cost
    an.update_generalized_link_cost(at, time_dependent, demand_period_id)  # 更新link_cost
    ...
    for c in an.get_centroids():
        ...
        single_source_shortest_path(an, node_id)  # 区域质点已经与区域所有点连接，可以找到以区域为源节点的最短路
        for c_ in an.get_centroids():  # 再次遍历所有区域，去掉与出发区域相同的
            if c_ == c:
                continue

            node_no = c_.get_node_no()
            to_zone_id = c_.get_zone_id()
            min_tt = an.get_node_label_cost(node_no)  # 目标区域的cost
